...
using namespace std;
#define N 10005
int n,line,from,to,head[N],last[N],which_line[N],cnt,m,k,belong[N],path[N],cost[N],ex[N];
struct node{
    int to;
    int line;
    int next;
...
bool bfs(){
    ...
    while(!Q.empty()){
        ...
        for(int i=head[cur.i];i;i=e[i].next){
            ...
            //int now_ex=e[i].line==cur.line?cur.ex:(cur.ex+1);
            int now_ex;
            if(e[i].line==cur.line) now_ex=cur.ex;
            else now_ex=cur.ex+1;
            Q.push({e[i].to,cur.i,cur.c+1,now_ex,e[i].line});
        }
    }
    return false;
}
